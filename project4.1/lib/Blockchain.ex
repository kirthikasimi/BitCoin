defmodule Blockchain do
    ...
    def compute_hash(%Blockchain{index: i, previous_hash: h, timestamp: timestamp, transactions: transactions, nonce: nonce}) do
    ...
    def add_txn(chain, transaction) do
      ...
      if transaction.from_address != nil do
        if(get_balance(chain, transaction.from_address) < transaction.amount + transaction.fee) do
          ...
          total = get_balance(chain, transaction.from_address) - Enum.reduce(chain.pending_txns, 0 , fn x, acc -> acc + x.amount + x.fee end) - transaction.amount - transaction.fee
          if total < 0 do
    ...
    def mining_pending_txns(blockchain, reward_address) do
      ...
      blockchain = add_txn(blockchain, new_txn(nil, reward_address, blockchain.mining_reward + mining_fee))
      ...
      blockchain = %{blockchain | chain: blockchain.chain ++ [b]}
      %{blockchain | pending_txns: []}
    end
    ...
    def merkle_tree(block, transactions) do
      if length(transactions) == 1 do
        ...
        transactions = if rem(length(transactions), 2) == 1 do
          transactions ++ [List.last(transactions)]
        else
          transactions
        end
        transactions = Enum.reduce(Enum.take_every(1..length(transactions), 2), [], fn x, acc -> acc ++ [:crypto.hash(:sha256, Enum.join([Enum.at(transactions, x),  Enum.at(transactions, x+1)])) |> Base.encode16] end)
    ...
    def mining_block(block, difficulty) do
      if String.slice(block.hash, 0..(difficulty - 1)) != String.duplicate("0", difficulty) do
        ...
        block = merkle_tree(block, Enum.map(block.transactions, fn x ->
          :crypto.hash(:sha256, Enum.join([x.from_address, x.to_address, x.amount, x.signature]))|> Base.encode16()
        end))
        block
      end
    end
