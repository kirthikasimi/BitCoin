defmodule BitcoinTest do
  ...
  test "Check block change in hash" do
    block = create_new_block(1, [], "")   
    block = %{block | timestamp: 123}          #Change the block and check change in hash
  ...
  test "Check validity of block on modification" do
    ...
    txn = %{txn| amount: 500}            #Modifying a completed transaction
  ...
  test "Check hashing for block hash" do
    ...
    hash=Enum.join([block.index, block.previous_hash, block.timestamp, Kernel.inspect(block.transactions), block.nonce])
  ...
  test "Check hashing for transaction hash" do
    ...
    assert hash == :crypto.hash(:sha256, Enum.join([txn.from_address, txn.to_address, txn.amount])) |> Base.encode16
  end
  ...
  test "Check balances after transaction" do
    ...
    txn = create_txn(node1, getwallet_address(node2), 10, 0)        #Node 1 sending 10 to Node 2 without fee
    blockchain = add_txn(blockchain, txn)
    blockchain = mining_pending_txns(blockchain, getwallet_address(node1)) 
  ...
  test "Check balance after transaction with fee" do
    ...
    node2 = new_node()
    blockchain = mining_pending_txns(blockchain, getwallet_address(node1))
    txn = create_txn(node1, getwallet_address(node2), 10, 10)        #Node 1 sending 10 to Node 2 fee 10
    blockchain = add_txn(blockchain, txn)
    blockchain = mining_pending_txns(blockchain, getwallet_address(node2))      #100-10-10
  ...
  test "Check mining reward balance" do
    ...
    txn = create_txn(node1, getwallet_address(node2), 10, 0)        #Node 1 sending 10 to Node 2 without fee
    blockchain = add_txn(blockchain, txn)
    blockchain = mining_pending_txns(blockchain, getwallet_address(node3))      #Mine and send reward to Node 3
  ...
  test "Checking enough balance for transaction" do
    ...
    node2 = new_node()
    blockchain = mining_pending_txns(blockchain, getwallet_address(node1))
    txn = create_txn(node1, getwallet_address(node2), 200)    #Node 1 sending 100 to node 2
    block = catch_throw(add_txn(blockchain, txn))
    assert block == "Insufficient Funds"
  end
  ...
  test "Check if all balance utilized" do
    ...
    blockchain = add_txn(blockchain, txn)
    txn = create_txn(node1, getwallet_address(node2), 100)      #Node 1 sending 100 to node 2
    block = catch_throw(add_txn(blockchain, txn))
